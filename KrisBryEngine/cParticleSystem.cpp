...
void cParticleSystem::update(double deltaTime) {
	...
	for (unsigned int index = 0; index != this->entitiesToUpdate.size(); ++index) {
		...
		glm::vec3 oldPosition = pParticleTrans->position;
		...
		for (unsigned int index = 0; index != numParticles; ++index) {
			...
			if (vecParticlesToDraw[index].lifeRemaining > 0.0f) {
				// Draw it
				pParticleTrans->position = vecParticlesToDraw[index].position;
				...
				cMeshRenderSystem::getInstance()->drawObject(pParticle, matIdentity, program);
				count++;
			}
		}
		...
		pParticleTrans->position = oldPosition;
